...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Приказ Минэнерго РФ № 326 от 30.12.2008\n\n"
        "📋 *Что умею:*\n"
        "1. Принимаю фото/скан однолинейной схемы\n"
        "2. Распознаю линии и трансформаторы через ИИ\n"
        "3. Учитываю топологию — параллельные и последовательные группы\n"
        "   (обозначаешь цветными маркерами прямо на схеме)\n"
        "4. Формирую Excel-шаблон с параметрами — проверяешь и вносишь правки\n"
        "5. Рассчитываю константы формул:\n"
        "   ΔW_лин = C × W²а / T\n"
        "   ΔW_тр  = A × T + B × W²а / T\n"
        "   ΔW     = ΔW_лин + ΔW_тр\n"
        "6. Считаю потери за любой месяц по показаниям счётчика\n\n"
...
async def cb_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.message.reply_text(
        ...
        "🎨 *Шаг 1 — Обозначь линии цветными маркерами:*\n"
        "• Параллельные линии — один цвет (например, все синим)\n"
        "• Другая параллельная группа — другой цвет (например, все красным)\n"
        "• Последовательные линии — каждая своим уникальным цветом\n"
        "• Трансформаторы цветом не выделяй — они всегда последовательны\n\n"
        "📌 *Пример:*\n"
        "  Главная КЛ-10 кВ → без цвета (последовательная)\n"
        "  ВЛ-0.4 от ТП-1 и ТП-2 параллельны → оба синим\n"
        "  ВЛ-0.4 от ТП-3 и ТП-4 параллельны → оба красным\n\n"
        "❗️ *Важно:*\n"
        "• Линии рядом с обводкой должны иметь подписи: марка, длина\n"
        "• Чем ярче и толще линия — тем точнее распознавание\n\n"
